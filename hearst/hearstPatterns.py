...
class HearstPatterns(object):
    ...
    def clean_hyponym_term(self, term):
        # print(term.replace("NP_", "").replace("_", " ").replace("such ", ""))

        # remove the "such" or "other"  if included with hyponym NP
        return term.replace("NP_", "").replace("_", " ").replace("such ", "").replace("other ", "")

if __name__=='__main__':
    hp = HearstPatterns(extended=False)
    # text = 'How do I distinguish among different musical genres such as blues, rock, and jazz, etc., and is there a good listeners’ trick to discern such distinctions?'
    # text = "We read genres from such authors as Rowling, Potter, and Granger"
    # text = "They come in with bruises, wounds, broken bones, or other injuries."
    # text = 'All commonlaw countries, including Canada, France and England are participating.'
    text = "this includes all European countries, especially France, England, or Spain"
